...
def build_header_inputs():
    ...
    other_pallet_type_1 = st.text_input("Specify pallet type", key="other_pallet_type_1") if pallet_type_1 == "Other" else ""
    ...
    if add_multiple_pallets:
        ...
        for i in range(1, num_additional + 1):
            ...
            other_pallet_type_i = st.text_input(f"Specify pallet type {i+1}", key=f"other_pallet_type_{i+1}") if pallet_type_i == "Other" else ""
    ...
    if "Transport / Cross Docking" in application:
        ...
        cross_docking_aisle = st.number_input(
            ...
            value=1.8,
            step=0.1,
            help="Recommended minimum 1.8 m for one-way driving/loading.",
    ...
    if "Stacking/Conveyor" in application:
        ...
        if stacking_type == "Floor Stacking":
            ...
            box_distance_mm = st.number_input(
                "Distance between 2 boxes stacked next to each other [mm]",
                min_value=200, value=200, step=1, key="box_distance_mm"
            )
            if box_distance_mm < 200:
                st.error("Minimum distance for floor stacking is 200 mm.")

            aisle_width_mm = st.number_input(
                "Floor Storage Aisle Width [mm]",
                min_value=1640, value=1640, step=1, key="aisle_width_mm"
            )
        ...
        if stacking_type == "Rack Stacking":
            box_distance_mm = st.number_input(
                "Distance between boxes on racks [mm]",
                min_value=75, value=75, step=1, key="box_distance_mm"
            )
            if box_distance_mm < 75:
                st.error("Minimum distance between pallets for rack stacking is 75 mm.")
            else:
                st.success(f"Larger distance ({box_distance_mm} mm) → faster process")

            aisle_width_mm = st.number_input(
                "Aisle Width for Rack Stacking [mm] (pallet to pallet)",
                min_value=2850, value=2850, step=1, key="aisle_width_mm"
            )
            if aisle_width_mm < 2850:
                st.error("Minimum aisle width for rack stacking is 2850 mm.")
    ...
    if any(app in application for app in ["Transport / Cross Docking", "Stacking/Conveyor", "Narrow Aisle"]):
        ...
        if pallets and pallets[0]["pallet_type"] == "Euro" and load_weight_kg > 1500:
            st.warning("Euro pallet cannot bear more than 1500 kg. Consider 'Other' type.")
    ...
    if "Stacking/Conveyor" in application or "Narrow Aisle" in application:
        ...
        if load_weight_kg <= 1200 and max_stacking_height_m > 4.5:
            st.warning("Standard max stacking height: 4.5 m for 1200 kg")
        elif load_weight_kg > 1200 and max_stacking_height_m > 3.5:
            st.warning("Max stacking height: 3.5 m for 1500 kg")
    ...
    with col_op1:
        ...
        pallets_per_day = st.number_input("Pallets per Day (peak)", min_value=0, value=50, key="pallets_per_day")
        shifts_per_day = st.number_input("Shifts per Day", 1, 3, value=2, key="shifts_per_day")

    with col_op2:
        hours_per_shift = st.number_input("Hours per Shift", min_value=0.0, value=8.0, step=0.5, key="hours_per_shift")
    ...
    # Platform clearance
    path_under_platform = st.radio("Are there any paths under the platform?", ["No","Yes"], key="path_under_platform")
    clearance_height_m = None
    if path_under_platform == "Yes":
        clearance_height_m = st.number_input("Height under platform [m]", min_value=0.0, value=2.5, step=0.1, key="clearance_height_m")

    # CAD upload
    cad_file = st.file_uploader("Upload CAD file, floor plan or layout", type=["dwg","pdf","png","jpg","jpeg","zip"], key="cad_layout_file")
    ...
    return {
        ...
        "aisle_width_m": aisle_width_m or 1.8,

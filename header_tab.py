...
def build_header_inputs():
    ...
    if "Stacking/Conveyor" in application:
        ...
        fork_entry_width = st.number_input("Fork Entry Width Required (min 320 mm)", min_value=0.0, value=320.0, key="fork_entry_width")

        pickup_type = st.radio("Pickup Type", ["Ground", "Conveyor"], key="pickup_type")

        if pickup_type == "Ground":
            stacking_type = st.radio("Stacking Type", ["Floor Stacking", "Rack Stacking"], key="stacking_type")

            if stacking_type == "Floor Stacking":
                storage_layout = st.text_area("Storage Layout Description", height=80, key="storage_layout")
                box_distance_mm = st.number_input("Distance between 2 boxes stacked next to each other [mm]", min_value=0.0, value=200.0, key="box_distance_mm")
                if box_distance_mm < 200:
                    st.error("Minimum distance between boxes for floor stacking is 200 mm. Not accepted.")
                aisle_width_mm = st.number_input("Aisle Width for Floor Stacking [mm]", min_value=0.0, value=1640.0, key="aisle_width_mm")
            ...
            if stacking_type == "Rack Stacking":
                box_distance_mm = st.number_input("Distance between boxes on racks [mm]", min_value=0.0, value=75.0, key="box_distance_mm")
                if box_distance_mm < 75:
                    st.error("Minimum distance between boxes for rack stacking is 75 mm. Not accepted.")
                aisle_width_mm = st.number_input("Aisle Width for Rack Stacking [mm] (pallet to pallet)", min_value=0.0, value=2850.0, key="aisle_width_mm")
        ...
        if pickup_type == "Conveyor":
            conveyor_height = st.number_input("Conveyor Height [mm]", min_value=0.0, value=0.0, key="conveyor_height")
            ...
            if load_at_edge == "No":
                distance_from_edge = st.number_input("Specify distance from conveyor edge to pallet [mm]", min_value=0.0, value=0.0, key="distance_from_edge")

    # ── Narrow Aisle ─────────────────────────────────────────────────────────────
    if "Narrow Aisle" in application:
        st.info("Narrow Aisle (XNA121 / XNA151): 1.78–2.0 m aisle width")
        aisle_width_m = st.number_input("Actual Aisle Width [m] (must be 1.78–2.0 m)", min_value=1.5, value=1.8, key="aisle_width_m")
    ...
    if any(app in application for app in ["Transport / Cross Docking", "Stacking/Conveyor", "Narrow Aisle"]):
        load_weight_kg = st.number_input("Load Weight [kg]", min_value=0.0, value=1000.0, key="load_weight_kg")
    ...
    if "Stacking/Conveyor" in application or "Narrow Aisle" in application:
        max_stacking_height_m = st.number_input("Maximum Stacking Height [m]", min_value=0.0, value=3.0, key="max_stacking_height_m")
    ...
    if "Stacking/Conveyor" in application:
        is_valid, msg, color = validate_xqe122(load_weight_kg, max_stacking_height_m, st.session_state.get("fork_entry_width", 320.0))
    ...
    if "Narrow Aisle" in application:
        is_valid, msg, color = validate_xna121_151(
            st.session_state.get("aisle_width_m", 1.8),
            load_weight_kg,
            max_stacking_height_m,
            st.session_state.get("xna_model", "XNA121 (up to 8.5m)")
        )
        if color == "red": st.error(msg)
        elif color == "orange": st.warning(msg)
        else: st.success(msg)
    ...
    with col_op1:
        ...
        max_transport_m = st.number_input("Average Transport Distance [m]", min_value=0.0, value=50.0, key="max_transport_m")
    ...
    return {
        ...
        "fork_entry_width": st.session_state.get("fork_entry_width", 320.0),
